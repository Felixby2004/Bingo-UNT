    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-4 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div className="max-h-[600px] overflow-y-auto divide-y divide-gray-50">
          {history.length === 0 ? (
            <div className="p-12 text-center text-gray-500 italic text-sm font-medium">No hay historial disponible</div>
          ) : (
            history.map(p => (
              <button 
                key={p.id}
                ...
                aria-label={`Ver detalles de la partida ${p.name}`}
              >
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    ...
                    <span className="text-[10px] text-gray-600 font-bold">{moment(p.finished_at).tz('America/Lima').format('DD/MM/YYYY')}</span>
                  </div>
                </div>
                <ChevronRight size={16} className={`text-gray-300 group-hover:text-unt-yellow transition-all ${selectedPrize?.prize.id === p.id ? 'translate-x-1 text-unt-yellow' : ''}`} />
              </button>
            ))
          )}
        </div>
        ...
        {selectedPrize ? (
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 p-8 space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-gray-50 pb-8">
              <div>
                <span className="inline-block bg-gray-100 text-gray-600 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest mb-3">Resumen de Partida</span>
                <h2 className="text-4xl font-black text-unt-blue uppercase tracking-tight mb-2">{selectedPrize.prize.name}</h2>
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-1 text-unt-yellow font-bold uppercase text-[10px]">
                    <Award size={14} />
              ...
              <div className="bg-green-50 px-6 py-4 rounded-3xl border border-green-100 flex items-center space-x-4">
                <div className="w-12 h-12 bg-green-100 rounded-2xl flex items-center justify-center text-green-700 shadow-inner">
                  <Trophy size={24} />
                </div>
                <div>
                  <p className="text-[10px] text-green-700 font-black uppercase tracking-widest leading-none mb-1">Ganador</p>
                  <p className="text-xl font-black text-green-800">{selectedPrize.prize.winner_name}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-black text-gray-600 uppercase tracking-widest">Números Cantados ({selectedPrize.drawnNumbers.length})</h3>
              </div>
